...
export default function ImportResult() {
  ...
  async function executeAdd() {
    try {
      const stored = sessionStorage.getItem('importResults');
      if (!stored) {
        router.push('/import');
        return;
      }

      const { results, syncMode, playlistName } = JSON.parse(stored);

      // Execute add
      const res = await fetch('/api/import/add', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tracks: results.map((r: any) => r.match),
          syncMode,
          playlistName,
        }),
      });
      ...
      sessionStorage.removeItem('importedSongs');
    } catch (err: any) {
    ...
    return (
      <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 items-center justify-center">
        <div className="text-center">
          <div className="text-xl font-semibold text-white mb-3">Adding tracks to Spotify...</div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-12 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-red-500/10 backdrop-blur-lg border border-red-500/30 rounded-2xl p-10 shadow-2xl">
            ...
            <h1 className="text-3xl font-bold text-red-400 mb-4">Failed to Add</h1>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold text-white mb-3">Import Complete!</h1>
          <p className="text-purple-200 text-lg">Your songs have been added to Spotify</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-500/10 to-blue-500/10 backdrop-blur-lg border border-green-500/30 rounded-2xl p-10 mb-8 text-center shadow-2xl">
          ...
          <h2 className="text-3xl font-bold text-white mb-8">Successfully imported!</h2>
